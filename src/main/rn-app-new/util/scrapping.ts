...
function fillFromSpotify($: cheerio.CheerioAPI, item: Item) {
    const authorPrefix = "song and lyrics by ";
    let author = $('head > title').text() || "";
    const authorIndex = author.indexOf(authorPrefix);
    if (authorIndex >= 0) {
        author = author.substring(authorIndex + authorPrefix.length);
    }
    item.author = removeSuffix(" | Spotify", author);
    item.tags = ["music", "top", "dance", "song"];
    item.notes = ""; // useless
}
...
function fillFromImdb($: cheerio.CheerioAPI, item: Item) {
...
function getMeta($: cheerio.CheerioAPI, metaType: string, name: string) : string {
